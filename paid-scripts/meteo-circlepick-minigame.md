...
### For Other Developer Scripts

<details>

<summary>qs-vehiclekeys/config/minigame.lua</summary>

update with this one for quasar vehiclekeys
...
-- 'meteo-circlepick' https://meteofivem.net/fivem-scripts/prodigy-rp-inspired-circlepick-minigame
Config.minigameType = 'meteo-circlepick'
...
function lockpickMiniGame(isAdvanced)
    ...
    local chosenDifficulty = difficultyArray[1] or 'easy'
    local timeLimits = { easy = 15, medium = 10, hard = 7 }
    local timeLimit = timeLimits[chosenDifficulty] or 10

    if Config.minigameType == 'meteo-circlepick' then
        success = exports['meteo-circlepick']:startLockpick(chosenDifficulty, timeLimit)
    end
