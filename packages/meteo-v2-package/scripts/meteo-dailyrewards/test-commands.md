---
description: >-
  showcase server commands for meteo daily rewards script. only available on the
  meteo showcase server.
metaLinks:
  alternates:
    - servers/meteo-fivem-server/documentation/scripts/meteo-dailyrewards/test-commands.md
---

# Test Commands

{% hint style="warning" %}
These commands are only available on our showcase server. use them to quickly test daily rewards features without waiting.
{% endhint %}

***

## Available Commands

### /dailycomplete \[taskIndex]

Complete a specific daily task by its index.

```
/dailycomplete 1
/dailycomplete 3
```

Task index is 1-7.

### /dailycompleteall

Complete all 7 daily tasks at once.

```
/dailycompleteall
```

### /dailyreset

Reset all daily task progress. test the daily cycle from scratch.

```
/dailyreset
```

### /dailynewtasks

Generate a fresh set of random daily tasks.

```
/dailynewtasks
```

### /dailyaddprogress \[amount]

Add progress to the first incomplete task.

```
/dailyaddprogress 5
```

### /dailygiveboost \[amount]

Give yourself an economy boost reward.

```
/dailygiveboost 1000
```

### /dailyboostreset

Reset your daily boost data. clears active boost and earned progress.

```
/dailyboostreset
```

| Command                | What it does           |
| ---------------------- | ---------------------- |
| **/dailycomplete**     | Complete one task      |
| **/dailycompleteall**  | Complete all tasks     |
| **/dailyreset**        | Reset progress         |
| **/dailynewtasks**     | Get new random tasks   |
| **/dailyaddprogress**  | Add progress to a task |
| **/dailygiveboost**    | Give boost reward      |
| **/dailyboostreset**   | Reset boost data       |

***

{% hint style="info" %}
Need help? contact Meteo support if you have any questions. get access to our exclusive test guide videos on our Discord.
{% endhint %}
